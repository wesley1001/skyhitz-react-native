...
var TabBar = React.createClass({
  ...
  getTabPlayerStyle(){
    return [
      styles.tabPlayer,
      {
        // opacity: this.state.pan.y.interpolate({inputRange: [-(Dimensions.get('window').height - 50),0], outputRange: [0,0.8]}),
  ...
  componentDidMount() {
    this.addListenerOn(Player.eventEmitter, 'loadVideo', this.setEntryTitle);
  ...
  playerBtn(){
    if(this.state.playerState == 'playing'){
      return(
          <TouchableOpacity onPress={Player.pauseCurrentVideo}>
          <Image style={styles.playBtn} source={require('image!pausebtnwhite')}/>
          </TouchableOpacity>
            )
    }else{
      return(
          <TouchableOpacity onPress={Player.playCurrentVideo}>
